...
void m3_test_op_metal_unsupported(m3_test_context *test)
{
    const uint64_t shape[] = {2U};
    const float input_values[] = {1, 2};
    const float sentinel[] = {-5, -5};
    ...
    M3_TEST_EXPECT(test, status == M3_STATUS_OK,
                   "create Metal backend for unsupported execution");
    if (backend == NULL) {
        return;
    }
    ...
    M3_TEST_EXPECT(test,
                   m3_storage_allocate(backend, sizeof(input_values), 16U,
                       ...
                       m3_storage_write(input_storage, 0U, input_values,
                                        sizeof(input_values), &error) ==
                           M3_STATUS_OK &&
                       m3_storage_write(output_storage, 0U, sentinel,
                                        sizeof(sentinel), &error) ==
                           M3_STATUS_OK &&
                       m3_tensor_view_contiguous(
                           &input, input_storage, M3_DTYPE_F32, 1U, shape, 0U,
                           &error) == M3_STATUS_OK &&
                       m3_tensor_view_contiguous(
    ...
    M3_TEST_EXPECT(test,
                   m3_backend_execute(backend, NULL, 0U, NULL, &error) ==
                       M3_STATUS_OK,
                   "empty command list is a no-op on Metal");
    M3_TEST_EXPECT(test,
                   m3_backend_execute(backend, &command, 1U, NULL, &error) ==
                       ...
                       memcmp(m3_storage_const_data(output_storage), sentinel,
                              sizeof(sentinel)) == 0,
                   "Metal rejects execution without CPU fallback or mutation");
