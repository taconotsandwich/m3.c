                                 ...
                                 m3_metal_dense_fixture *fixture)
{
    ...
    if (!m3_op_test_fixture_init(&fixture->host)) {
        M3_TEST_EXPECT(test, false, "create dense host fixture");
        return false;
    }
...
static bool m3_metal_dense_strided_one(
...
bool m3_metal_dense_strided(m3_metal_dense_fixture *fixture,
                            ...
                            const void *values)
{
    return m3_metal_dense_strided_one(
               &fixture->host, &view->host, dtype, rank, shape, strides,
               byte_count, values) &&
           m3_metal_dense_strided_one(
...
{
    const uint8_t *host = m3_storage_const_data(view->host.storage);
    const uint8_t *metal = m3_storage_const_data(view->metal.storage);

    return view->host.metadata.byte_count == view->metal.metadata.byte_count &&
           memcmp(host + view->host.byte_offset,
