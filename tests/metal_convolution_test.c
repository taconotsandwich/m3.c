...
bool m3_metal_convolution_fixture_init(
    m3_test_context *test, m3_metal_convolution_fixture *fixture)
{
    ...
    if (!m3_op_test_fixture_init(&fixture->host)) {
        M3_TEST_EXPECT(test, false, "create convolution host fixture");
        return false;
    }
    ...
    M3_TEST_EXPECT(test, status == M3_STATUS_OK,
                   "create convolution Metal fixture");
...
void m3_metal_convolution_fixture_dispose(
...
bool m3_metal_convolution_tensor(
    m3_metal_convolution_fixture *fixture,
    m3_metal_convolution_view *view, m3_dtype dtype, uint8_t rank,
...
static bool m3_metal_convolution_strided_one(
...
bool m3_metal_convolution_strided(
    m3_metal_convolution_fixture *fixture,
    m3_metal_convolution_view *view, m3_dtype dtype, uint8_t rank,
    const uint64_t *shape, const size_t *strides, size_t byte_offset,
    size_t byte_count, const void *values)
{
    return m3_metal_convolution_strided_one(
               &fixture->host, &view->host, dtype, rank, shape, strides,
               byte_offset, byte_count, values) &&
           m3_metal_convolution_strided_one(
               &fixture->metal, &view->metal, dtype, rank, shape, strides,
               byte_offset, byte_count, values);
}

bool m3_metal_convolution_execute(
    m3_metal_convolution_fixture *fixture,
    const m3_command *host_commands, const m3_command *metal_commands,
    size_t command_count, m3_error *error)
{
    return m3_backend_execute(fixture->host.backend, host_commands,
                              command_count, NULL, error) == M3_STATUS_OK &&
           m3_backend_execute(fixture->metal.backend, metal_commands,
                              command_count, NULL, error) == M3_STATUS_OK;
}

bool m3_metal_convolution_equal(
    const m3_metal_convolution_view *view)
{
    const uint8_t *host;
    const uint8_t *metal;
