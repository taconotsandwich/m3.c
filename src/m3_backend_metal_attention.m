...
NSString *m3_metal_source_attention(void)
{
    NSString *support =
        @"#pragma clang fp contract(off)\n"
            ...
            "}\n"
            "ulong m3_attention_mask_offset(\n"
            "    constant M3AttentionParameters& p, ulong batch,\n"
            "    ulong head, ulong query, ulong key) {\n"
            ...
            "* p.mask.shape[2] + mask_query) * p.mask.shape[3] + key;\n"
            ...
            "    constant M3AttentionParameters& p, ulong batch,\n"
            "    ulong query_head, ulong kv_head, ulong query, "
            "ulong key) {\n"
            ...
            "    ulong query_flat = (((batch * query_heads + "
            "query_head) * query_count + query) * depth) + channel;\n"
            "    ulong key_flat = (((batch * kv_heads + kv_head) * "
            "key_count + key) * depth) + channel;\n"
            "    ulong query_offset = "
            "m3_view_logical_offset(p.query, query_flat);\n"
            "    ulong key_offset = "
            "m3_view_logical_offset(p.key, key_flat);\n"
            "    float query_value = "
            ...
            "  sum = sum * p.scale;\n"
            "  if (p.has_mask != 0u) {\n"
            "    ulong mask_offset = m3_attention_mask_offset("
            "p, batch, query_head, query, key);\n"
            "    float mask_value = "
    ...
    NSString *kernel =
        @"kernel void m3_attention(\n"
            ...
            "mask_data, p, batch, query_head, kv_head, query, key);\n"
            ...
            "query_data, key_data, mask_data, p, batch, query_head, "
            "kv_head, query, key);\n"
            ...
            "        ulong value_flat = (((batch * kv_heads + kv_head) "
            "* key_count + key) * depth) + channel;\n"
            "        ulong value_offset = "
            "m3_view_logical_offset(p.value, value_flat);\n"
            "        float value = "
