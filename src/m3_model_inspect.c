...
static m3_status m3_scan_weight_names(const char *component_path,
                                      m3_component_id id,
                                      ...
                                      m3_error *error)
{
    ...
    if (status == M3_STATUS_OK && shards->count > 1U && *index_name == NULL) {
        status = m3_error_set(error, M3_STATUS_INVALID_FORMAT,
                              "sharded component '%s' lacks an index",
                              component_path);
    }
    if (status == M3_STATUS_OK && shards->count == 1U &&
        *index_name != NULL) {
        status = m3_error_set(error, M3_STATUS_INVALID_FORMAT,
                              "single-file component '%s' has an index",
                              component_path);
    }
    return status;
}
...
static m3_status m3_validate_shard_tensors(
    ...
    const m3_model_index *index, m3_string_list *tensor_names,
    m3_component_id id, m3_model_metadata *metadata, m3_error *error)
{
    ...
    for (tensor_index = 0U;
        ...
        status = m3_string_list_add(
            tensor_names, shard.tensors[tensor_index].name, true, error);
        indexed_shard = index == NULL
                            ? NULL
                            : m3_model_index_shard(
                                  index, shard.tensors[tensor_index].name);
        if (status == M3_STATUS_OK && index != NULL &&
        ...
        if (status == M3_STATUS_OK) {
            status = m3_model_metadata_add_tensor(
                metadata, id, shard.tensors[tensor_index].tensor.byte_count,
                error);
        }
    }
    ...
    m3_error *error)
{
    ...
    m3_string_list tensor_names = {NULL, 0U, 0U};
    ...
    for (shard_index = 0U;
        ...
        status = m3_validate_shard_tensors(
            ...
            &tensor_names, id, metadata, error);
    }
    if (status == M3_STATUS_OK && active_index != NULL &&
        active_index->entry_count != tensor_names.count) {
        status = m3_error_set(error, M3_STATUS_INVALID_FORMAT,
                              "model index tensor count does not match shards");
    }
    if (status == M3_STATUS_OK && active_index != NULL &&
        active_index->has_total_size &&
        active_index->total_size !=
            (uint64_t)metadata->components[(size_t)id].tensor_bytes) {
        status = m3_error_set(error, M3_STATUS_INVALID_FORMAT,
                              "model index total_size does not match shards");
    }
    m3_model_index_dispose(&index);
    m3_string_list_dispose(&tensor_names);
                                     ...
                                     m3_error *error)
{
    ...
    for (component_index = 0U;
        ...
        if (m3_component_contains_weights(id)) {
            status = m3_inspect_weight_component(path, id, &inspected, error);
        } else {
