...
static NSString *m3_metal_common_source(void)
{
    NSString *prefix =
        @"#include <metal_stdlib>\n"
         ...
         "struct M3ViewParameters {\n"
         "  ulong element_count; ulong byte_offset;\n"
         "  ulong shape[8]; ulong byte_strides[8];\n"
         "  uint rank; uint dtype;\n"
         "};\n"
         "static_assert(sizeof(M3ViewParameters) == 152, "
         "\"Metal view parameter size changed\");\n"
         "static_assert(alignof(M3ViewParameters) == 8, "
    ...
    NSString *numeric = [prefix stringByAppendingString:
        @"ushort m3_f32_to_f16_bits(uint bits) {\n"
         ...
         "}\n"
         "float m3_round_float(uint type, float value) {\n"
         "  uint bits = as_type<uint>(value);\n"
         "  if (type == 0u) return value;\n"
         "  if (type == 1u) return as_type<float>(\n"
         "    m3_f16_to_f32_bits(m3_f32_to_f16_bits(bits)));\n"
         "  if (type == 2u) return as_type<float>(\n"
         "    uint(m3_f32_to_bf16_bits(bits)) << 16);\n"
    ...
    return [numeric stringByAppendingString:
        @"ulong m3_logical_offset(ulong flat, ulong base, "
         ...
         "}\n"
         "ulong m3_view_logical_offset(\n"
         "    constant M3ViewParameters& view, ulong flat) {\n"
         "  if (flat >= view.element_count) return ~ulong(0);\n"
         "  return m3_logical_offset(flat, view.byte_offset, view.shape,\n"
         "    view.byte_strides, view.rank);\n"
         "}\n"
         "ulong m3_view_broadcast_offset(\n"
         "    constant M3ViewParameters& view,\n"
         "    constant M3ViewParameters& output, ulong flat) {\n"
         "  ulong invalid = ~ulong(0);\n"
         "  if (view.rank > output.rank || output.rank > 8u ||\n"
         "      flat >= output.element_count ||\n"
         "      view.element_count == 0u) return invalid;\n"
         "  ulong offset = view.byte_offset;\n"
         "  uint leading = output.rank - view.rank;\n"
         "  for (uint axis = output.rank; axis > 0u; --axis) {\n"
         "    uint output_axis = axis - 1u;\n"
         "    ulong output_dimension = output.shape[output_axis];\n"
         "    if (output_dimension == 0u) return invalid;\n"
         "    ulong coordinate = flat % output_dimension;\n"
         "    flat /= output_dimension;\n"
         "    if (output_axis >= leading) {\n"
         "      uint view_axis = output_axis - leading;\n"
         "      ulong view_dimension = view.shape[view_axis];\n"
         "      if (view_dimension != 1u &&\n"
         "          view_dimension != output_dimension) return invalid;\n"
         "      if (view_dimension == 1u) coordinate = 0u;\n"
         "      ulong stride = view.byte_strides[view_axis];\n"
         "      if (coordinate != 0u &&\n"
         "          stride > (invalid - offset) / coordinate)\n"
         "        return invalid;\n"
         "      offset += coordinate * stride;\n"
         "    }\n"
         "  }\n"
         "  return flat == 0u ? offset : invalid;\n"
         "}\n"
         "ulong m3_view_contiguous_offset(\n"
         "    constant M3ViewParameters& view, ulong flat) {\n"
         "  ulong invalid = ~ulong(0);\n"
         "  if (flat >= view.element_count || view.dtype > 3u)\n"
         "    return invalid;\n"
         "  ulong element_size = ulong(m3_dtype_size(view.dtype));\n"
         "  if (flat != 0u &&\n"
         "      element_size > (invalid - view.byte_offset) / flat)\n"
         "    return invalid;\n"
         "  return view.byte_offset + flat * element_size;\n"
         "}\n"];
}
